...
```bash
brew tap user/tap
brew install --cask agent-gateway
```
...
swift run agent-gateway --version | tail -n 1 | grep -Fx "$version"
```
...
```text
dist/homebrew-cask/agent-gateway-<version>-darwin-arm64.dmg
dist/homebrew-cask/agent-gateway-<version>-darwin-x64.dmg
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/agent-gateway-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/agent-gateway-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/agent-gateway-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/agent-gateway-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/agent-gateway.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/agent-gateway.rb README.md
git diff --staged --stat
git commit -m "chore: release agent-gateway <version>"
...
```bash
brew fetch --cask user/tap/agent-gateway
HOMEBREW_NO_GITHUB_API=1 brew audit --cask user/tap/agent-gateway
```
...
verify `api/cask/agent-gateway.json` from GitHub
