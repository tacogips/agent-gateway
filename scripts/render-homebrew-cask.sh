...
main() {
  ...
  release_base_url="${CASK_RELEASE_BASE_URL:-https://github.com/tacogips/agent-gateway/releases/download/v$version}"
...
cask "agent-gateway" do
  ...
  url "$release_base_url/$artifact_name-#{version}-#{arch}.dmg",
      verified: "github.com/tacogips/agent-gateway/releases/download/"
  name "agent-gateway"
