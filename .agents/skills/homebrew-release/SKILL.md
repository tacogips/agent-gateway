...
| --- | --- |
| macOS Apple Silicon | `agent-gateway-<version>-darwin-arm64.tar.gz` |
| macOS Intel | `agent-gateway-<version>-darwin-x64.tar.gz` |
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
scripts/render-homebrew-formula.sh "$version" /path/to/homebrew-tap/Formula/agent-gateway.rb
```
...
brew install user/tap/agent-gateway
agent-gateway --version
brew test user/tap/agent-gateway
```
...
verify `api/formula/agent-gateway.json` from
